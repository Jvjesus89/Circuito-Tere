...
export async function listUsuarios(req, res, next) {
  try {
    const result = await pool.query(
      'SELECT idusuario, usuario, senha, isadministrador, email, telefone, cpf FROM public.usuarios ORDER BY idusuario ASC'
    );
...
export async function createUsuario(req, res, next) {
  try {
    ...
    if (usuario == null || senha == null || email == null) {
      return res
        .status(400)
        .json({ message: 'Campos obrigatórios: usuario, senha, email' });
    }
    ...
    if (err?.code === '23505') {
      ...
      return res.status(409).json({
        message: 'Violação de unicidade (usuario/email/cpf já existente)',
      });
    }
    next(err);
  }
...
export async function updateUsuario(req, res, next) {
  try {
    const { id } = req.params;
    const { usuario, senha, isadministrador, email, telefone, cpf } =
      req.body || {};
    ...
    if (isadministrador !== undefined) {
      fields.push(`isadministrador = $${idx++}`);
      values.push(Boolean(isadministrador));
    }
    if (email !== undefined) {
...
export async function deleteUsuario(req, res, next) {
  try {
    const { id } = req.params;
    const result = await pool.query(
      'DELETE FROM public.usuarios WHERE idusuario = $1',
      [Number(id)]
    );
    if (result.rowCount === 0) {
      return res.status(404).json({ message: 'Usuário não encontrado' });
 ...
 * Importante: atualmente a senha é armazenada em texto puro.
 * Em produção, o ideal é usar hash (bcrypt) e tokens JWT.
 */
export async function loginUsuario(req, res, next) {
  try {
    ...
    const query = `
      SELECT idusuario, usuario, senha, isadministrador, email, telefone, cpf
    ...
    const safeUser = mapUsuarioRow(user);

    // Aqui poderíamos gerar um token JWT; por enquanto retornamos apenas os dados do usuário.
    return res.json({
